...
for(ii in unique(doc$legislature)) {
  ...
  if(plot) {
    ...
    ggnet_save(n, file = paste0("plots/net_fr_se", ii),
  ...
  if(gexf) {
    ...
    get_gexf(paste0("net_fr_se", ii), n, c(meta[1], "Sénat"), mode, colors,
...
save(list = ls(pattern = "^(net|edges|bills)_fr_se\\d{1,2}$"), file = "data/net_fr_se.rda")
